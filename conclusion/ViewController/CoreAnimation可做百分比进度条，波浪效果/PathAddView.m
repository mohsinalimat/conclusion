...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"2"]]];
        
        a = 1.5;
        b = 0;
        jia = NO;
        
        _currentWaterColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"1"]];
//        _currentWaterColor = [UIColor redColor];
        
//        _currentLinePointY = self.frame.size.height;
        _currentLinePointY = 40;
...
- (void)drawRect:(CGRect)rect {
    ...
    for(float x=0;x<=DeviceWidth;x++){
        y= a * sin( x/180*M_PI + 4*b/M_PI ) * 5 + _currentLinePointY;
        CGPathAddLineToPoint(path, nil, x, y);
    }
    
    CGPathAddLineToPoint(path, nil, DeviceWidth, rect.size.height);
