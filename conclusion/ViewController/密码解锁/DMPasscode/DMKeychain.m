...
@implementation NSObject (DMKeychainPropertyListCoding)

- (id)DMKeychain_propertyListRepresentation
{
    return self;
}
...
#if !DMKeychain_USE_NSCODING

@implementation NSNull (DMKeychainPropertyListCoding)
...
- (id)DMKeychain_propertyListRepresentation
{
    ...
    for (id obj in self)
    {
        id value = [obj DMKeychain_propertyListRepresentation];
...
+ (instancetype)defaultKeychain
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedInstance = [[DMKeychain alloc] initWithService:bundleID
                                                 accessGroup:nil];
    });
          ...
          accessGroup:(NSString *)accessGroup
{
    return [self initWithService:service
                   ...
                   accessibility:DMKeychainAccessibleWhenUnlocked];
}
...
- (NSData *)dataForKey:(id)key
{
    ...
    {
		NSLog(@"DMKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
	}
...
- (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    if (object && !data)
    {
        data = [NSPropertyListSerialization dataWithPropertyList:[object DMKeychain_propertyListRepresentation]
    ...
    NSAssert(!object || (object && data), @"DMKeychain failed to encode object for key '%@', error: %@", key, error);
    ...
    if (data)
    {
        ...
        {
            NSLog(@"DMKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    else if (self[key])
    {
        ...
        if (status != errSecSuccess)
        {
            NSLog(@"DMKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    return YES;
...
- (id)objectForKey:(id)key
{
    ...
    if (data)
    {
        ...
        if (!object)
        {
             NSLog(@"DMKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    }
    else
